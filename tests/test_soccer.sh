...
do
    ...
    python run.py soccer $agent -n 5 -e 2

    printf "\n----- ($agent agent) Test BPR opponent -----\n"
    python run.py soccer $agent -n 5 -b

    printf "\n----- ($agent agent) Test Phi opponent -----\n"
    for i in {1..11}
    do
        printf "\n----- ($agent agent, Phi opponent) Phi $i -----\n"
        python run.py soccer $agent -n 5 --phi-opponent --phi $i
    done
    ...
    do
        ...
        python run.py soccer $agent -n 5 --new-phi-opponent -q $q
    done
    ...
    do
        ...
        python run.py soccer $agent -n 5 --new-phi-noise-opponent -pat 0.$p
    done
done
