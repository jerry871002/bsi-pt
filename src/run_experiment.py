    ...
    if args.multi_processing:  # multi-processing mode
        ...
        with multiprocessing.Manager() as manager:
            ...
            with multiprocessing.Pool() as pool:
                result = manager.dict(
                    {agent: manager.list() for agent in run_functions.keys()}
                )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run multiple runs of the experiment in a given scenario with an agent type')
    parser.add_argument(
    ...
    parser.add_argument(
        '-a', '--agents', type=str, nargs='*', choices=('bpr+', 'deep-bpr+', 'tom', 'bpr-okr', 'bsi', 'bsi-pt'),
    ...
    parser.add_argument(
        '-m', '--multi-processing', action='store_true',
        help='whether to compute using multi-processing'
    )
    parser.add_argument(
        '-d', '--data-dir', type=Path, default=Path('data/'),
