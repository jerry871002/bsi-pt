...
class NavigationGame:
    def __init__(
        ...
        # set goals
        self.G1 = (0, self.height // 2)
        self.G2 = (2, self.height // 2)
        self.G3 = (self.width // 2, self.height // 2)
        self.G4 = (self.width - 3, self.height // 2)
        self.G5 = (self.width - 1, self.height // 2)

        # set rewards and punishment
        self.reward_G1 = reward_G1
        self.reward_G2 = reward_G2
        self.reward_G3 = reward_G3
        self.reward_G4 = reward_G4
        self.reward_G5 = reward_G5
    ...
    def generate_performance_model(self) -> np.ndarray:
        ...
        for agent_policy in list(agent.Policy):
            for opponent_policy in list(opponent.Policy):
                ...
                env.reset()
                while True:
                    done, reward, _, _ = env.step(agent.get_action())
                    rewards += reward
                    if done:
                        break
    ...
    def reset(self) -> State:
        self.agent.set_xy(self.width // 2, 0)
        self.opponent.set_xy(self.width // 2, self.height - 1)
    ...
    def step(self, agent_action: Move) -> Tuple[bool, int, State, Tuple[Move, Move]]:
        ...
        if not done:
            # the agent gets an r_step = -1 punishment when moving
    ...
    def reach_goal(self, location: Location) -> bool:
        return True if location in (self.G1, self.G2, self.G3, self.G4, self.G5) else False
    ...
    def game_over(self, agent_action: Move, opponent_action: Move) -> Tuple[bool, int]:
        ...
        agent_G1 = agent_loc_ == self.G1 or agent_loc == self.G1
        agent_G2 = agent_loc_ == self.G2 or agent_loc == self.G2
        agent_G3 = agent_loc_ == self.G3 or agent_loc == self.G3
        agent_G4 = agent_loc_ == self.G4 or agent_loc == self.G4
        agent_G5 = agent_loc_ == self.G5 or agent_loc == self.G5

        opponent_G1 = opponent_loc_ == self.G1 or opponent_loc == self.G1
        opponent_G2 = opponent_loc_ == self.G2 or opponent_loc == self.G2
        opponent_G3 = opponent_loc_ == self.G3 or opponent_loc == self.G3
    ...
    def show(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                # draw the goals
                if (x, y) in (self.G1, self.G2, self.G3, self.G4, self.G5):
                    if (x, y) == self.agent.get_xy():
                        ...
                        print(goals_display.get((x, y)), end='')
                    continue
...
class Opponent(Agent):
    ...
    def get_action(self) -> Move:
        ...
        if self.policy is self.Policy.ONE:
            ...
            #               *
            #               s
            if self.get_xy() == (7, 8): return Move.UP
            if self.get_xy() == (7, 7): return Move.UP
            if self.get_xy() == (7, 6): return Move.UP
            if self.get_xy() == (7, 5): return Move.UP
            ...
            #               s
            if self.get_xy() == (7, 8): return Move.UP
            if self.get_xy() == (7, 7): return Move.LEFT
            if self.get_xy() == (6, 7): return Move.LEFT
            if self.get_xy() == (5, 7): return Move.UP
            if self.get_xy() == (5, 6): return Move.LEFT
            if self.get_xy() == (4, 6): return Move.LEFT
            if self.get_xy() == (3, 6): return Move.UP
            if self.get_xy() == (3, 5): return Move.LEFT
            if self.get_xy() == (2, 5): return Move.UP
            ...
            #               s
            if self.get_xy() == (7, 8): return Move.UP
            if self.get_xy() == (7, 7): return Move.RIGHT
            if self.get_xy() == (8, 7): return Move.RIGHT
            if self.get_xy() == (9, 7): return Move.UP
            if self.get_xy() == (9, 6): return Move.RIGHT
            if self.get_xy() == (10, 6): return Move.RIGHT
            if self.get_xy() == (11, 6): return Move.UP
            if self.get_xy() == (11, 5): return Move.RIGHT
            if self.get_xy() == (12, 5): return Move.UP
            ...
            #         *
            #         * * * s
            if self.get_xy() == (7, 8): return Move.LEFT
            if self.get_xy() == (6, 8): return Move.LEFT
            if self.get_xy() == (5, 8): return Move.LEFT
            if self.get_xy() == (4, 8): return Move.UP
            if self.get_xy() == (4, 7): return Move.UP
            if self.get_xy() == (4, 6): return Move.LEFT
            if self.get_xy() == (3, 6): return Move.LEFT
            if self.get_xy() == (2, 6): return Move.LEFT
            if self.get_xy() == (1, 6): return Move.LEFT
            if self.get_xy() == (0, 6): return Move.UP
            if self.get_xy() == (0, 5): return Move.UP
            ...
            #                     *
            #               s * * *
            if self.get_xy() == (7, 8): return Move.RIGHT
            if self.get_xy() == (8, 8): return Move.RIGHT
            if self.get_xy() == (9, 8): return Move.RIGHT
            if self.get_xy() == (10, 8): return Move.UP
            if self.get_xy() == (10, 7): return Move.UP
            if self.get_xy() == (10, 6): return Move.RIGHT
            if self.get_xy() == (11, 6): return Move.RIGHT
            if self.get_xy() == (12, 6): return Move.RIGHT
...
def get_terminal_state_combination(ternimal_state: Tuple[Location, Location]) -> Tuple[bool]:
    ...
    return (
        opponent_location == G1 and agent_location == G1,
        opponent_location == G2 and agent_location == G2,
        opponent_location == G3 and agent_location == G3,
        opponent_location == G4 and agent_location == G4,
        opponent_location == G5 and agent_location == G5,
        opponent_location == G1 and agent_location != G1,
        opponent_location == G2 and agent_location != G2,
        opponent_location == G3 and agent_location != G3,
        opponent_location == G4 and agent_location != G4,
