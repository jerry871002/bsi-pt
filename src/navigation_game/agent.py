...
class BprAgent(Agent):
    ...
    def get_action(self) -> Move:
        ...
        if self._policy is self.Policy.ONE:
            ...
            if (x, y) == (7, 3) or (x in (2, 12) and y < 3):
                return Move.DOWN
            if y == 0:
        ...
        raise ValueError(f'BprAgent with policy {self._policy} shouldn\'t be in {self.get_xy()}')
...
class DeepBprPlusAgent(BprAgent):
    ...
    def compute_tau_hat(self, state: Location):
        ...
        if state == (0, 4):
            self.tau_hat = np.array([0, 0.0, 0.0, 1.0, 0.0])
        elif state == (2, 4):
...
class BsiBaseAgent(BprAgent):
    ...
    def update_phi(self) -> None:
        ...
        for observation in self.observation_model[self.policy.value - 1]:
            observation_model_unnormalized.append(
                np.array_equal(observation, np.array(sigma).reshape(2, 2))
            )
